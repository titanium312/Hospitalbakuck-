...
export class Preguntasfrecuentes extends LitElement {
  ...
  constructor() {
    ...
    this.faqs = [
      { q: '¿Cómo puedo agendar una cita?', a: 'Puedes agendar por teléfono, en ventanilla o a través del portal web (si tu asegurador lo permite). Ten a la mano tu documento de identidad y datos de contacto. Para especialidades, puede requerirse remisión.' },
      { q: '¿Dónde consulto resultados de Ayudas Diagnósticas?', a: 'En el portal de resultados de Ayudas Diagnósticas (si aplica) o presencialmente en la sede indicada. Verifica horarios y requisitos en la sección de Resultados del sitio.' },
      { q: '¿Cómo solicito copia de la historia clínica?', a: 'La historia clínica es un documento reservado. Debes radicar la solicitud con tu identificación; terceros necesitan autorización y soportes de parentesco. Consulta la sección “Historia clínica” para requisitos.' },
      { q: '¿Qué hago en caso de urgencia?', a: 'Acudir directamente al servicio de Urgencias más cercano. La atención no requiere autorización previa. Lleva tu documento y, si es posible, información clínica relevante (alergias, medicamentos).' },
      { q: '¿Cómo radico una PQRSDF?', a: 'Puedes usar nuestro canal en línea o radicar presencialmente. Recibirás un número de radicado para seguimiento. Conserva ese número y consulta el estado en el mismo canal.' },
      { q: '¿Qué documentos debo llevar a mi cita?', a: 'Documento de identidad del paciente; si es menor de edad, del acudiente. En algunos servicios, soporte de afiliación y/o remisión del profesional tratante.' },
      { q: '¿Cuál es el horario de atención?', a: 'Horario administrativo habitual: lunes a viernes 7:00 a. m. – 5:00 p. m. Consultar cada servicio (laboratorio, imágenes, consulta externa) pues pueden variar.' },
      { q: '¿Dónde están ubicadas las sedes?', a: 'Revisa la sección “Sedes y contacto” para direcciones, mapas y rutas de transporte. También puedes llamar a nuestra central telefónica para orientación.' },
      { q: '¿Quién emite certificados y constancias?', a: 'Estos documentos se expiden por el servicio tratante. Verifica requisitos, tiempos de entrega y si requieren pago de copagos o estampillas según normativa vigente.' },
      { q: '¿Cómo reporto un acto de corrupción?', a: 'Usa la Línea de transparencia: tienes opciones por teléfono, formulario web y correo. El reporte puede ser confidencial y anónimo.' }
    ];
  }
  ...
  _onSearchInput(e) { this.filter = e.target.value; }

  _expandAll() { this.querySelectorAll('details').forEach(d => d.open = true); }
  _collapseAll() { this.querySelectorAll('details').forEach(d => d.open = false); }
  ...
  render() {
    ...
    return html`
      <!-- La barra TTS “afuera” envuelve todo el contenido visible -->
      <dictador-tts ui lang="es-CO" rate="1" pitch="1" volume="1">
        ...
        <section class="faq-wrap w-100 pa3 pa4-ns">
          ...
          <div class="flex items-center flex-column flex-row-ns gap2 mb3">
            ...
            <input
              id="faq-search"
              class="input-reset ba b--black-10 br2 pa2 w-100 w-50-ns"
              type="search"
              placeholder="Buscar (ej.: urgencias, historia clínica)"
      ...
      <details id="${id}" class="pa3 pa4-ns">
        <summary class="flex items-center justify-between">
          ...
          <div class="flex items-center">
            <button
              class="link-btn mr2"
              title="Copiar enlace a esta pregunta"
              @click=${(e) => { e.preventDefault(); e.stopPropagation(); this._copyLink(id); }}
            >

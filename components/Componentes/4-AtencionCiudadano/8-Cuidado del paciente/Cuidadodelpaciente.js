    ...
    return html`
      <dictador-tts ui lang="es-CO" rate="1" pitch="1" volume="1">
        <style>
          ...
          .cdp-title { letter-spacing: -0.02em; }
          .cdp-kicker {
            font-weight: 600; color: var(--cdp-accent-strong);
            text-transform: uppercase; letter-spacing: .08em;
          }
          .cdp-section {
            background: var(--cdp-bg); border: 1px solid var(--cdp-border);
            border-radius: .75rem; transition: transform .12s ease, box-shadow .12s ease;
          }
          .cdp-section:hover { transform: translateY(-1px); box-shadow: 0 10px 24px rgba(2, 8, 23, 0.06); }
          .cdp-badge {
            display: inline-block; border-radius: 999px; padding: .25rem .6rem;
            font-size: .75rem; font-weight: 600; background: #e2f6ff;
            color: var(--cdp-accent-strong); border: 1px solid #cdefff;
          }
          .cdp-list li + li { margin-top: .5rem; }
          .cdp-muted { color: var(--cdp-muted); }
          .cdp-cta {
            display: inline-flex; align-items: center; gap: .5rem;
            border-radius: .75rem; border: 1px solid var(--cdp-border);
          ...
          .cdp-cta--primary {
            background: var(--cdp-accent); color: #fff; border-color: var(--cdp-accent);
          }
          .cdp-cta--primary:hover { background: var(--cdp-accent-strong); border-color: var(--cdp-accent-strong); }
          .cdp-help {
            background: var(--cdp-surface); border: 1px dashed var(--cdp-border); border-radius: .75rem;
          }
          .cdp-grid { display: grid; grid-template-columns: 1fr; gap: 1rem; }
          @media (min-width: 40rem) {
            .cdp-grid { grid-template-columns: repeat(3, 1fr); }
            .cdp-grid-2 { grid-template-columns: repeat(2, 1fr); }
          }
          .cdp-icon { inline-size: 1.25rem; block-size: 1.25rem; }
          /* Accesibilidad: foco visible */
          .cdp-cta:focus-visible, a:focus-visible, button:focus-visible {
            outline: 3px solid #94d9ff; outline-offset: 2px; border-radius: .75rem;
          }
        </style>
        ...
        <section class="cdp-wrap pa4 pa5-ns">
          ...
          <header class="cdp-header br3 pa4 mb4">
            ...
            <h1 class="cdp-title f2 f1-ns lh-title ma0">Cuidado del paciente</h1>
          ...
          <div class="cdp-help pa3 pa4-ns mb4">
            ...
            <p class="ma0 cdp-muted">
              Teléfono: <strong>(+00) 000 0000</strong> • Correo:
              <a class="link dim underline" href="mailto:contacto@tuhospital.gov.co">contacto@tuhospital.gov.co</a>
            </p>
          </div>

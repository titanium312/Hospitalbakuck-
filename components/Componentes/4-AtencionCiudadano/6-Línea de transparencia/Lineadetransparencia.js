  ...
  render() {
    ...
    return html`
      <style>
        ...
        .ldx-ribbon { height: 6px; background: linear-gradient(90deg, var(--ldx-accent), #63a4df 60%, #9ec7ef); border-top-left-radius: 1rem; border-top-right-radius: 1rem; }
        .ldx-badge { background: var(--ldx-accent-weak); color: var(--ldx-accent); border: 1px solid rgba(43,123,187,.25); }
        .ldx-card { border: 1px solid rgba(43,123,187,.18); border-radius: 1rem; box-shadow: 0 6px 24px rgba(43,123,187,.08); background: #fff; transition: transform .25s ease, box-shadow .25s ease, border-color .25s ease; }
        .ldx-card:hover, .ldx-card:focus-within { transform: translateY(-4px); border-color: rgba(43,123,187,.35); box-shadow: 0 10px 28px rgba(43,123,187,.14); }
        ...
        .ldx-subcard { border: 1px solid rgba(43,123,187,.18); border-radius: .75rem; background: #fff; }
        .btn { display:inline-flex; align-items:center; gap:.5rem; border:1px solid rgba(0,0,0,.08); border-radius:8px; padding:8px 12px; background:#F9FCFF; color:#084b83; text-decoration:none; font-weight:600; transition: background-color .2s ease, transform .05s ease; }
        .btn:hover { background: var(--ldx-accent-weak); }
        .btn:active { transform: scale(.98); }
        @keyframes ldx-fadeUp { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
        .ldx-anim { animation: ldx-fadeUp .35s ease both; }
        @media (prefers-color-scheme: dark) { .ldx-transparencia { background: #ffffff; color: var(--ldx-text); } .ldx-card, .ldx-subcard { background: #fff; border-color: rgba(43,123,187,.18); } }
      </style>

      <!-- Barra TTS EXTERNA: lee todo lo visible de este módulo -->
